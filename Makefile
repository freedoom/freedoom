...
# Clean files produced by "wad-diff".
wad-diff-clean:
	rm -f wad-images/*.png

clean: wad-diff-clean
...
# Generate a visual diff of the level changes.
wad-diff:
	scripts/wad-diff $(commit) $(wad_regex)

# Display what was generated by "wad-diff".
wad-diff-display: wad-diff
	@echo "Displaying WAD diffs."
	@if ls wad-images/*diff*.png &> /dev/null; \
	then \
		display wad-images/*diff*.png; \
	fi
