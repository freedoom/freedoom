...
Freedoom est disponible sous la licence BSD, permettant
n'importe qui de partager, de modifier et de réutiliser des parties du jeu.

Pour plus de détails, consultez la <<reusing,section sur la réutilisation>>.
...
Pour jouer au jeu, vous pouvez utiliser l’une des nombreuses adaptations ("portage")
du programme Doom que les fans ont développé pour les ordinateurs modernes.

Le Doom Wiki possède une
https://doomwiki.org/wiki/Source_port[page dédiés aux portages], où vous pouvez trouver
...
* Placez les fichiers `.wad` de Freedoom dans le même dossier que le portage
  avant de le lancer. Il est possible qu'ils soient automatiquement détectés.
* Si vous le lancez à partir de la ligne de commande, essayez
  ex. `mon-portage-favoris -iwad freedoom1.wad`.
...
Ça revient à 62 niveaux à jouer, et il y a aussi des niveaux secrets -- si
vous pouvez découvrir comment les atteindre.
...
| **Read This!** | Affiche un écran d'aide montrant une description des
objets que vous verrez dans le jeu.
...
|==========================
| 1 | **Please Don’t Kill Me!** | Le niveau de difficulté le plus facile. C’est la même chose que _Will This Hurt?_, sauf que les dégâts sont divisé par deux.
...
[**Attention:** le programme Doom originale contient un bug qui le fait planter lorsque vous sauvegarder une partie avec trop de choses qui se passent dans le niveau. Le portage Chocolate Doom réplique ce bug intentionnellement. Si ceci est indésirable, veuillez aller dans chocolate-setup et désactiver "Vanilla savegame limit" avant de jouer.]

=== Quitter le jeu

Lorsque vous aurez fini de jouer à Freedoom, appuyez sur _Échap_ pour afficher le menu principal et sélectionnez _Quit Game_ pour quitter. Vous aurez surement envie de sélectionner _Save Game_ en premier afin d'enregistrer votre progrès et pouvoir revenir à l’endroit où vous vous étiez arrêté lors de votre prochaine session.

=== Raccourcis clavier

Quelques raccourcis clavier pratiques qui vous aideront à gagner du temps pour accéder aux fonctions courantes du menu.
...
| **F8** | Messages | Bascule entre montrer ou masquer les messages à l’écran qui s'affiche lorsque vous ramassez un objet.
...
Voici les contrôles de base du jeu pour  interagir avec l'environnement:
[options="header",cols="3,1,2,1",width="70%",align="center",halign="center"]
|==========================
| Fonction | Touche par Defaut ^1^ 1 | Touche par Defaut 2 | Souvent assigné à ^2^
| Déplacer avant/arrière | Haut/Bas | Déplacement de la Souris (ou Mouse2 pour avancé) | Z/S
| Déplacer ("strafe") gauche/droite | ,/. | Alt (ou Mouse3) + Gauche/Droite | Q/D
| Tourner gauche/droite ^3^ | Gauche/Droite | Déplacement de la Souris | Déplacement de la Souris
| Tirer | Ctrl | Mouse1 | Mouse1
| Utiliser | Espace | Double-click Mouse2 ou Mouse3 | E
| Courir ^4^ | Shift | - | Shift
|==========================

^1^ Votre portage devrait vous donner la possibilité d’utiliser n’importe quel bouton sur votre manette ou votre souris; pour être plus brief, le terme "touche" est utilisé dans ce manuel pour inclure d’autres commandes selon les circonstances.

^2^ Sur un clavier AZERTY les touches Z, S, Q et D constituent un deuxième ensemble de touches directionnelles pour la main gauche.

^3^ Si vous avez un monstre, un baril ou un adversaire en PvP qui traverse le milieu de votre écran lors du coup de feu de votre arme, le jeu ajustera votre visée verticale pour vous. Certains portages vous permettront de désactiver ce comportement et de viser manuellement à la place.

^4^ La plupart des portages ont une option "Toujours Courrir" ("Always Run" en Anglais) où le fait de maintenir cette touche ralentit votre vitesse de déplacement. Le personnage du joueur ne se fatigue pas donc la vitesse lente est seulement nécessaire pour améliorer la précision.

**Les défauts de Doom sont largement considérés suboptimaux;** Vérifiez votre portage pour savoir comment les reconfigurer. Les options les plus fréquemment utilisés sont fournies, mais il n’y a pas de "meilleure" solution qui convient pour tout le monde -- vous allez peut-être devoir expérimenter. Au minimum, vous devez être à l’aise en vous déplaçant dans l’une des quatre directions tout en tournant et en tirant simultanément.
...
* Déplacez-vous -- sans tourner -- afin que votre pistolet soit pointé vers 
  l'autre colonne (Points bonus si vous pouvez vous arrêter naturellement sur la cible.)
...
* Essayez de faire un carré (ou le numéro huit, etc.) tout en pointant
  vers la colonne en même temps. Privilégiez la fluidité au lieu de la précision -- il vaut mieux être proche la plupart du temps que parfait une partie du temps.

* Déplacez-vous sur un des lits dans les coins afin que la colonne ne
  soit plus visible. Déplacez-vous afin que la colonne entre et sorte de vue. Expérimentez avec la distance et le timing. Et tentez de pointer vers la colonne même si vous ne pouvez pas la voir.
...
* Explorez le reste de la zone. Vous trouverez deux portes, une de
  chaque type: celle dans la tranchée peut être utilisée directement, comme l'ascenseur, tandis que celle en haut est ouverte à l'aide d'un interrupteur à proximité. Une fois que vous avez fait votre choix, ouvrez la porte et préparez-vous à utiliser le savoir que vous avez acquis.
...
| **Health** | Si ceci atteint zéro, vous êtes mort! Consultez la <<health,section santé>> pour les power-ups qui peuvent être trouver pour rétablir votre santé.
...
Dans le jeu, vous trouverez divers objets ramassable: <<weapons,des armes>>, <<ammo,des munitions>>, <<health,de la santé>>, <<armor,de l'armure>>, <<keys,des clés>> et quelques <<specialitems,power-ups plus rares>> qui vous donnent des capacités spéciales.

Pour ramasser quelque chose, il suffit tout simplement de marcher dessus -- un message à l’écran et un bref flash indiquent que vous l’avez bien ramassé. Si vous ne le ramassez pas, il est probable que vous n’en ayez pas besoin actuellement (par exemple, vous ne pouvez pas prendre d'objets qui rechargent votre santé lorsque votre santé est déjà à 100%). Si un objet vous donne plus que ce que vous pouvez transporter, la différence est perdue.
...
| **Poing** | 1 | Si vous n’avez plus de munitions, vous pouvez toujours frapper les monstres à mains nues. Munitions : Aucune
| **Scie** +
image:../sprites/csawa0.png[Ripsaw] |
1 | Conçue pour couper le bois, la scie fonctionne également comme une arme de corps à corps pour trancher la chair. Munitions : Aucune
| **Pistolet** +
image:../sprites/pista0.png[Handgun] |
2 | Votre arme de départ. Elle vous permet de vous battre pour obtenir une meilleure arme, et d'activer des interrupteurs sans gaspiller de balle. Munitions : Balles
| **Fusil à Pompe** +
image:../sprites/shota0.png[Pump-action Shotgun] |
3 | Tire sept cartouches qui se propagent, vous permettant de toucher plusieurs cibles ou une grande cible. Munitions : Cartouches
| **Fusil à Double Canon** +
image:../sprites/sgn2a0.png[Double-barrelled Shotgun] |
3 | Une meilleure tolérance pour les charges puissantes signifie une meilleure fragmentation des projectiles, ce qui se traduit par une augmentation de près de 50% du nombre de coups par cartouches sur une zone plus étendue. Bon à courte portée contre les groupes d’ennemis. Munitions : Cartouches
| **Minigun** +
image:../sprites/mguna0.png[Minigun] |
4 | Une utilisation bien plus efficace de vos balles qu'avec le pistolet. Jusqu’à quarante secondes de souffrance pour vous protéger. Munitions : Balles
| **Lance-Missile** +
image:../sprites/launa0.png[Missile Launcher] |
5 | Tire des missiles qui infligent beaucoup de dégâts à l’impact, puis explose pour éliminer les petits monstres à proximité. Attention, éviter d'être trop proche de l’explosion! Munitions : Missiles
...
7 | Une arme expérimentale qui tire une boule d’énergie polarique massive, puis déclenche une explosion d’énergie secondaire dans la même direction! Tire lentement, mais ça vaut la peine. Munitions : Énergie
|==========================
...
Vous commencez avec votre santé à 100%. Vous mourrez lorsque votre santé atteint 0%.

En ramassant n'importe quel objet de santé, vous récupérerez la quantité de vie indiquée, jusqu’à sa limite. Les recharges sont limitées à 100%, mais les boosts (1% et 100%) sont limitées à 200%.
...
L'armure normale absorbe un tiers des dégâts que vous recevez. L'absorption est arrondie vers le bas : si vous avez 100% de vie et 100% d'armure, et que vous subissez 50% de dégâts, vous perdrez 34% de vie et 16% d'armure.
...
Les clées dans Freedoom sont non seulement conçues pour être distinguer par leur couleur mais aussi par leur forme, afin de rendre le jeu plus accessible aux joueurs daltoniens. Chaque clé colorée a une forme unique associée :
...
Ces formes sont utilisées de façon consistante tout au long du jeu : avec les icônes de la barre d’état, les images des clés et sur les murs indiquant les portes verrouillées.
...
Il est possible que vous voyiez l'un de ces objets spéciaux lorsque vous explorez ;

[cols="1,2",width="90%",align="center",valign="middle"]
|==========================
| **Lunettes Vision Sombre** +
image:../sprites/pvisa0.png[Low-Light Goggles] |
Permet de voir dans le noir, pour un temps limité.
| **Carte Enquête Zone** +
image:../sprites/pmapa0.png[Area Survey Map] |
Révèle les zones inexplorées de la carte, y compris les zones secrètes qui ne sont pas immédiatement visibles.
| **Combinaison de Secours** +
image:../sprites/suita0.png[Rescue Operations Suit] |
Protège contre la chaleur, les toxines et les radiations provenant des sols endommagés, pour un temps limité.
| **Symbiote de la Force** +
image:../sprites/pstra0.png[Strength Symbiote] |
Rétablis votre santé à 100%, et vos poings font 10x plus de dégâts jusqu’à la fin du niveau.
| **Cape d'Invisibilité** +
image:../sprites/pinsa0.png[Invisibility Cloak] |
Vous rend presque invisible pour un temps limité. Les monstres détectent toujours votre présence, mais ils auront beaucoup plus de mal à viser.
| **Surtension Néguentropique** +
image:../sprites/megaa0.png[Negentropic Surge] |
Maximise votre santé et armure à 200%.
| **Dispositif Vanguard** +
image:../sprites/pinva0.png[Vanguard Device] |
Vous immunise contre tous les dégâts pour un temps limité, vous permettant de surmonter les défenses écrasantes de l'ennemie et les pièges inévitables.
|==========================
...
| **Ver Furtif** +
Ces variantes du Ver de Chair ont été équipé avec des capacités furtives qui les rendent pratiquement invisibles. |
image:images/monster-stealth-worm.png[Stealth Worm,100,100,width=100%]
| **Larvillon** +
Larves aliens flottantes qui vous chargent à distance. |
...
Si le Procureur de Douleur n'était pas assez balaise, celui-ci peut encaisser cinq missiles. |
...
| **Limace de Combat** +
Ces monstres de chair génétiquement modifiés ont été équipés de lance-flammes à longue portée, ce qui en fait des chars vivants et glissants. |
...
Lors de l'utilisation de la carte, le jeu n'est pas mis sur pause. Et les contrôles continuent de fonctionner comme d'habitude, mais les touches supplémentaires suivantes sont disponibles:
...
La lave rouge brûlante et la boue radioactive ne sont que deux des types de sols nocifs que vous pouvez rencontrer dans les niveaux de Freedoom. Si vous devez marcher dessus, essayez de trouver une <<specialitems,combinaison de secours>>, mais sachez qu'elle ne vous protégera que pour un temps limité. |
...
Si vous gallerez avec la difficulté du jeu, ca vaux peut-ête le coup d'examiner les suggestions suivantes:

* Prenez le temps pour configurer vos contrôles -- pour l'assignation des
...
* Jouez avec un casque. La séparation stéréo du jeu peut vous donner des
...
* Parfois, vous affronterez des foules de monstres, ce qui peut être
  épuisant et drainer vos munitions. Apprenez à maîtriser le contrôle des foules. L'instinct primaire de tous les monstres est de se diriger vers vous. Tournez continuellement autour de la foule -- cela les incite à se regrouper dans un seul endroit, plus facile à cibler. Cela encourage également les monstres à se battre ; lorsque effectuer efficacement, ils dépenseront leur énergie à s'entretuer et vous économiserez sur vos munitions.
...
[[wads]]
== Jouer avec des WADS et mods fait par les fans ==

.Joue à Scythe MAP09 avec Freedoom.
image::images/scythe-map09.png[Scythe MAP09,width="640",pdfwidth="70vw",align="center"]

Une des meilleures fonctionnalités de Freedoom est sa compatibilité avec le catalogue de milliers de niveaux créés par les fans pour les jeux classiques _Doom_ . À quelques exceptions près, les mods et niveaux les plus populaires pour _Doom_ et _Doom II_ peuvent également être joués avec Freedoom. Le plus grand répertoire de mods _Doom_ est l'archive idgames, une interface de navigation pour l'archive https://www.doomworld.com/idgames/[peut être trouvée sur Doomworld].

Charger un fichier `.wad` est généralement assez simple. Pour les mods fabriqués pour le premier _Doom_, utilisez Freedoom: Phase 1 (`freedoom1.wad`); pour les autres fabriqués pour _Doom 2_ ou _Final Doom_, utilisez Freedoom: Phase 2 (`freedoom2.wad`).
Si vous vous servez de la ligne de commande, utilisez le paramètre `-file` lorsque vous démarrez le jeu. Par exemple, pour charger le fichier `scythe.wad`:

  mon-portage-favori -iwad freedoom2.wad -file scythe.wad

Si vous utilisez pas la ligne de commande, vous pouvez cliquer glisser le fichier `.wad` sur l'icône de votre portage dans votre gestionnaire de fichiers -- plusieurs portages supporte cette fonctionnalité.
...
* https://www.doomworld.com/cacowards/[Les Cacowards] est une cérémonie annuelle organisée par Doomworld, avec une remise de prix qui récompense les meilleures sorties de la communauté _Doom_ au cours de la dernière année. C'est un bon moyen de découvrir les développements plus récents, y compris les mods les plus inhabituels que les gens publient.

* La https://doomwiki.org/wiki/List_of_notable_WADs[Liste des WADs notables] du Doom Wiki contient une liste exhaustive de WADs fait par les fans. Le Doom Wiki comprend des informations détaillées sur de tels mods, ce qui comprends des captures d'écran, des cartes et des statistiques par niveau, c'est donc un point d'entrée utile pour découvrir des mods intéressants.
...
Si vous trouvez le jeu trop difficile, vous pouvez toujours jouer à un <<skill,niveau de difficulté plus facile>>. Cependant, si cela ne suffit pas, ou si vous voulez expérimenter avec les mécaniques du jeu, il y a quelques triches que vous pouvez utiliser:
...
| **IDCLIP** | Mode Noclip, vous permet de traverser les murs
...
Si vous souhaitez contribuer au projet Freedoom, veuillez consulter https://github.com/freedoom/freedoom[la page du projet], https://www.doomworld.com/forum/17-freedoom/[le forum de discussion] et https://discord.gg/9DA3fut[le chat discord].

https://help.github.com/en/github[Comment utiliser le contrôle de version Git pour des contributions]

https://guides.github.com/activities/forking/[Comment faire un "fork" d'un projet et crée un "pull request" avec Git]
