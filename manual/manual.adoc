...
when you save a game while too much is going on on the level. Some
source ports intentionally emulate this behaviour. You may want to check
your source port's compatibility settings and disable this (e.g.,
"Vanilla savegame limit" in Chocolate Doom) before playing.]
...
on the skull - blue diagonal, yellow vertical, red horizontal: +
...
Freedoom is a volunteer project that receives contributions from a wide
variety of people with differing abilities and needs. The community
tries to address accessibility issues as they come up but there is always
room for improvement.

Contributions and feedback are welcome.

* **Color blind accessibility:** Nothing should be distinguished solely
by hue. Each type of key has different markings, the two armors have
different silhouettes, and switches have up/down positions; where color
is the main difference, it always accompanies a change in brightness as
well. (The "pain brothers" are confusingly similar even for people who
are not color blind. :))

* **Controls:** Areas that can only be accessed by straferunning are
discouraged, as it may not be available on certain controller configurations.
Recent efforts are being made to ensure that no obligatory components
require precisely timed combinations of movements.
All levels are tested to avoid "hot starts" where enemies can attack
players before the player has provided any input.

* **Cognitive:** Levels are regularly tested for signposting and navigability.
In-game messages are given key words to distinguish what sort of gameplay
mechanic is being affected, minimizing lookup time.
The same word is generally used for the same thing.
